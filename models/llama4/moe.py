...
class Experts(nn.Module):
    ...
    def load_hook(
        ...
        if prefix + "moe_w_in_eD_F" in state_dict:
            ...
            state_dict[prefix + "w2"] = state_dict.pop(prefix + "moe_w_out_eF_D").view(
                e, -1, D
            )
            state_dict[prefix + "w3"] = state_dict.pop(
                prefix + "moe_w_swiglu_eD_F"
            ).view(e, D, -1)
    ...
    def batched_swiglu(self, x: Tensor, w1: Tensor, w3: Tensor, w2: Tensor) -> Tensor:
        print(
            f"======x.shape={x.shape} w1.shape={w1.shape} w3.shape={w3.shape} w2.shape={w2.shape}"
        )
...
class MoE(torch.nn.Module):
    ...
    def __init__(
        ...
        self.router_DE: nn.Parameter = nn.Parameter(
            torch.empty(dim, moe_args.num_experts, dtype=dtype)
        )
    ...
    def load_hook(
        ...
        if prefix + "w_in_shared_FD.weight" in state_dict:
            state_dict[prefix + "shared_expert.w1.weight"] = state_dict.pop(
                prefix + "w_in_shared_FD.weight"
            )
            state_dict[prefix + "shared_expert.w3.weight"] = state_dict.pop(
                prefix + "w_swiglu_FD.weight"
            )
            state_dict[prefix + "shared_expert.w2.weight"] = state_dict.pop(
                prefix + "w_out_shared_DF.weight"
            )
    ...
    def forward(self, x_bsD: Tensor) -> Tensor:  # noqa: N803
        ...
        router_indices = (
            torch.arange(a, device=x_aD.device)
            .view(1, -1)
