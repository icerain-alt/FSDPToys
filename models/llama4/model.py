...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.attn_temperature_tuning and not self.use_rope:
            seq_positions = torch.arange(seqlen, device=xq.device, dtype=torch.float32)
            attn_scales = (
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.dim = args.dim
        self.head_dim = (
            args.dim // args.n_heads if args.head_dim is None else args.head_dim
        )
        ...
        if (
            args.moe_args
            and (layer_id + 1) % args.moe_args.interleave_moe_layer_step == 0
        ):
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.tensor) -> torch.tensor:
        ...
        if seqlen > 1:
            global_attn_mask = torch.full(
                (seqlen, seqlen), float("-inf"), device=tokens.device
            )
            global_attn_mask = torch.triu(global_attn_mask, diagonal=1).type_as(h)

            if chunk_size := self.args.attention_chunk_size:
                local_attn_mask = create_chunked_attention_mask(
                    seqlen, chunk_size, tokens.device
                )

        for layer in self.layers:
            h = layer(h, freqs_cis, global_attn_mask, local_attn_mask)
        h = self.norm(h)
...
def create_chunked_attention_mask(
    seq_len: int, attention_chunk_size: int, device: torch.device
) -> torch.Tensor:
    block_pos = torch.abs(
